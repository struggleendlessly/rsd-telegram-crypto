...
async Task<string> PerformEthCall(Web3 web3)
{
    ...
    var token0 = contract.ContractBuilder.ContractABI.Functions.Where(x => x.Name.Equals("token0", StringComparison.InvariantCultureIgnoreCase)).FirstOrDefault().Sha3Signature;
    var token1 = contract.ContractBuilder.ContractABI.Functions.Where(x => x.Name.Equals("token1", StringComparison.InvariantCultureIgnoreCase)).FirstOrDefault().Sha3Signature;
