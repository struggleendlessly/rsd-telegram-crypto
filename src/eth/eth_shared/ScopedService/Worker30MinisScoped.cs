...
{
    public sealed class Worker30MinisScoped : IScopedProcessingService
    {
        ...
        private const string schedule = "0/30 * * * *"; // every 30 min
        private readonly CronExpression _cron;

        public Worker30MinisScoped(
            ILogger<Worker30MinisScoped> logger,
            IsDead isDead,
            GetPair getPair,
            dbContext dbContext,
        ...
        public async Task DoWorkAsync(CancellationToken stoppingToken)
        {
            ...
            {
                _logger.LogInformation("Worker Worker30MinisScoped running");
                ...
                _logger.LogInformation("Worker Worker30MinisScoped running at: {time}", DateTimeOffset.Now);
                ...
                {
                    _logger.LogError("Worker Worker30MinisScoped Exception: {message}", ex.Message);
                    _logger.LogError("Worker Worker30MinisScoped Exception: {stack}", ex.StackTrace);
                }

                var timeEndStep1 = DateTimeOffset.Now;

                _logger.LogInformation("Worker Worker30MinisScoped running time: {time}", (timeEndStep1 - timeStartStep1).TotalSeconds);

                await Task.Delay(1_800_000, stoppingToken);
            }
        }
        ...
        async Task Start()
        {
            ...
            {
                _logger.LogInformation("Worker Worker30MinisScoped volumePrepare running at: {time}", DateTimeOffset.Now);
                ...
                _logger.LogInformation("Worker Worker30MinisScoped volumePrepare .Start(30)");
                ...
                _logger.LogInformation("Worker Worker30MinisScoped volumePrepare running time: {time}", (timeEnd - timeStart).TotalSeconds);
            }
        }
    }
}
